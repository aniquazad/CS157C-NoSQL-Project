...
def main_menu():
    options = """
    **************************************************************************
    Please enter the number of the option/operation you would like to run.
    1. Add article
    ...
    Enter (q/Q/any non-numerical key) to exit
    **************************************************************************"""
    print(options)
...
def addArticle():
    ...
    'headline':{'main':headline},'keywords':keywords,
    'pub_date':pub_date,'document_type':'article', 'section_name':section_name,'subsection_name': subsection_name, 
    'byline':byline,'type_of_material':type_of_material,'_id':article_id,'word_count':word_count,
    'uri':article_id, 'read_count':0}
...
def readAbstractBasedOnExpr():
    expr = input('Enter an expression contained in the abstract:\t')
...
def getTypeOfMaterialAndMultimedia():
    print("\n[Op-Ed, News, Letter, Schedule, Brief, Editorial, Review, Correction, Obituary (Obit), Slideshow]")
    material = input('Type of Material:\t')
    query = {'type_of_material': material, 'multimedia': {'$exists': True}}
...
def getNMostPopularKeywords():
    ...
    query = {'keywords': {'$slice':top_n_kw}, '_id':0,'abstract':0, 'headline':0,
    'byline':0,'snippet':0,'lead_paragraph':0,'multimedia':0, 'pub_date':0, 
    'news_desk':0, 'section_name':0,'uri':0, 'word_count':0,'type_of_material':0,
    'document_type':0,'read_count':0,'source':0}
